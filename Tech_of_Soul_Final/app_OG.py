...
.evt-list-item:hover { background: rgba(201,168,76,0.06); }
.evt-list-name { font-size: 0.9rem; color: var(--text); }
.evt-list-meta { font-family: 'Space Mono', monospace; font-size: 0.65rem; color: var(--muted); }
...
visible_events = [
    e for e in EVENTS
    if e["year_start"] <= year and e["category"] in selected_cats
]
...
for cat in CATEGORIES:
    if cat in selected_cats:
        ...
        legend_html += f'<div class="legend-item"><div class="legend-dot" style="background:{col}"></div><span class="legend-txt">{cat}</span></div>'
legend_html += '</div>'
...
fig.update_layout(
    geo=dict(
        showframe=False,
        showcoastlines=True, coastlinecolor="#2A2A4A", coastlinewidth=0.8,
        showland=True, landcolor="#151530",
        showocean=True, oceancolor="#080818",
        showlakes=True, lakecolor="#0D0D22",
        showcountries=True, countrycolor="#1A1A38", countrywidth=0.5,
...
for e in visible_events:
    for sp in e.get("spread", []):
        if sp["year"] <= year:
            age_factor = max(0.1, 1 - (year - sp["year"]) / 80)
            fig.add_trace(go.Scattergeo(
                lon=[e["lon"], sp["lon"]],
                lat=[e["lat"], sp["lat"]],
                mode="lines",
                line=dict(width=1.2, color=e["color"]),
                opacity=0.25 * age_factor,
                hoverinfo="skip",
                showlegend=False,
            ))
            # Spread dot
            fig.add_trace(go.Scattergeo(
                ...
                mode="markers",
                marker=dict(
                    size=7, color=e["color"],
                    symbol="circle",
                    opacity=0.55,
                    line=dict(width=1, color="#0A0A12")
                ),
                text=f"{e['name']} → {sp['city']}",
...
for e in visible_events:
    ...
    pulse = max(12, min(28, 10 + age * 0.25))
    is_selected = st.session_state.selected_event == e["id"]
    fig.add_trace(go.Scattergeo(
        lon=[e["lon"]], lat=[e["lat"]],
        mode="markers+text",
        marker=dict(
            size=pulse if not is_selected else pulse + 5,
            color=e["color"],
            symbol="circle",
            opacity=0.9,
            line=dict(width=2 if is_selected else 1, color="#FFFFFF" if is_selected else "#0A0A12")
        ),
        text=e["name"] if age < 5 else "",
...
fig.add_annotation(
    ...
    showarrow=False,
    font=dict(family="Cinzel Decorative", size=36, color="rgba(201,168,76,0.18)"),
    align="left"
)
...
with map_col:
    st.markdown(f'<div class="counter-badge">✦ {len(visible_events)} movements active in {year}</div>', unsafe_allow_html=True)

    click = st.plotly_chart(fig, width="stretch", key="main_map", on_select="rerun", selection_mode="points")
...
with panel_col:
    ...
    for e in sorted(visible_events, key=lambda x: x["year_start"]):
        ...
        border = f"border-left: 3px solid {e['color']};" if is_sel else f"border-left: 2px solid {e['color']}33;"
        bg = "background: rgba(255,255,255,0.04);" if is_sel else ""
        if st.button(
            f"{'▶ ' if is_sel else ''}{e['name']} ({e['year_start']})",
            key=f"btn_{e['id']}",
            width="stretch",
        ):
            st.session_state.selected_event = e["id"] if not is_sel else None
            st.rerun()
    ...
    if ev:
        ...
        with img_col:
            try:
                ...
                c = ev["color"]
                st.markdown(f"<div style='background:{c}22;border:1px solid {c}44;height:180px;border-radius:4px;display:flex;align-items:center;justify-content:center;color:{c};font-family:Cinzel Decorative;font-size:2rem'>✦</div>", unsafe_allow_html=True)

        with info_col:
            tags_html = "".join(f'<span class="tag">{t}</span>' for t in [ev["category"], ev["origin_city"]])
            st.markdown(f"""
<div class="event-panel">
  <h3>{ev['name']}</h3>
  <div class="event-year">◈ {ev['year_start']} — {"present" if ev['year_end'] >= 2024 else ev['year_end']} &nbsp;|&nbsp; {ev['origin_city']}</div>
  {tags_html}
