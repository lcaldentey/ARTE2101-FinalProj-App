...
EVENTS = [
  {
    ...
    "description": (
        "Emanuel Swedenborg was a celebrated Swedish polymath — engineer, anatomist, cosmologist — "
        ...
        "exploration. His multi-volume works described the architecture of heaven and hell in "
        ...
        "divine truth — a hermeneutical move that licensed countless later esotericists. His rejection "
        "of substitutionary atonement and his vision of a God of love rather than judgment made his "
  ...
  },
  {
    ...
    "description": (
        ...
        "simultaneously spectacle, medical innovation, and popular sensation. Moving to Paris in 1778, "
        "Mesmer attracted aristocratic patrons before a royal commission (including Benjamin Franklin "
        "and Lavoisier) debunked animal magnetism in 1784. The debunking did not kill the movement. "
  ...
  },
  {
    ...
    "description": (
        ...
        "fervent, experiential Christianity. The Jesus Movement emerged from youth hostels, "
        "coffeehouses, and beach baptisms in California and spread nationally within a few years. Its "
        "central innovation was applying the countercultural model to evangelism: informal dress, rock "
        "music, communal living, and the language of 'experience' and 'encounter' transmitted an "
        "essentially conservative Protestant theology. Lonnie Frisbee — a charismatic figure who "
        "performed mass baptisms in the Pacific — was the movement's most electric figure, though his "
        ...
        "Jesus Movement produced the entire genre of Contemporary Christian Music: Larry Norman, "
        "Keith Green, and later Amy Grant translated the emotional register of rock and pop into "
        "Christian devotion."
    ),
...
if "selected_events" not in st.session_state: st.session_state["selected_events"] = set()
if "detail_focus"   not in st.session_state: st.session_state["detail_focus"]   = None  # last-clicked, for detail panel
...
def interactive_body():
    ...
    sel_ids  = st.session_state["selected_events"]
    has_sel  = len(sel_ids) > 0
        ...
        if click and click.get("selection") and click["selection"].get("points"):
            ...
            if pts:
                ...
                if cdata:
                    ...
                    sel = st.session_state["selected_events"]
                    if eid in sel:
                        sel.discard(eid)
                        # If we just removed the detail-focus, shift to another selected one
                        if st.session_state["detail_focus"] == eid:
                            st.session_state["detail_focus"] = next(iter(sel), None)
                    else:
                        sel.add(eid)
                        st.session_state["detail_focus"]  = eid
                        st.session_state["scroll_to_det"] = True
                    st.session_state["selected_events"] = sel
    ...
    with panel_col:
        ...
        for ev in sorted(visible, key=lambda x: x["year_start"]):
            ...
            if st.button(label, key=f"btn_{ev['id']}", use_container_width=True):
                ...
                if is_sel:
                    sel.discard(ev["id"])
                    if st.session_state["detail_focus"] == ev["id"]:
                        st.session_state["detail_focus"] = next(iter(sel), None)
                else:
                    sel.add(ev["id"])
                    st.session_state["detail_focus"]  = ev["id"]
                    st.session_state["scroll_to_det"] = True
                st.session_state["selected_events"] = sel

    # ── Detail panel ──────────────────────────────────────────────────────────
    focus_id = st.session_state["detail_focus"]
    # Keep focus_id in sync: if the focused event left the visible set, pick another selected one
    if focus_id is not None and focus_id not in {ev["id"] for ev in visible}:
        remaining = sel_ids & {ev["id"] for ev in visible}
        focus_id  = next(iter(remaining), None)
        st.session_state["detail_focus"] = focus_id

    if focus_id is not None:
        det = next((e for e in EVENTS if e["id"] == focus_id), None)
        if det:
            st.markdown("---")
            scroll_js = ""
            ...
            # Tab row if multiple movements are selected
            visible_sel = [ev for ev in visible if ev["id"] in sel_ids]
            if len(visible_sel) > 1:
                tab_html = '<div style="display:flex;gap:0.4rem;flex-wrap:wrap;margin-bottom:0.6rem">'
                for ev in sorted(visible_sel, key=lambda x: x["year_start"]):
                    active  = ev["id"] == focus_id
                    bg      = ev["color"] + ("CC" if active else "22")
                    border  = ev["color"]
                    txt_col = "#0A0A12" if active else ev["color"]
                    tab_html += (
                        f'<span style="background:{bg};border:1px solid {border};color:{txt_col};'
                        f'font-family:Space Mono,monospace;font-size:0.6rem;letter-spacing:0.07em;'
                        f'padding:0.2rem 0.55rem;border-radius:2px;cursor:default">'
                        f'{ev["name"]}</span>'
                    )
                tab_html += '</div>'
                st.markdown(tab_html, unsafe_allow_html=True)
                # Actual clickable tab buttons (invisible — tabs above are decorative labels)
                tab_cols = st.columns(len(visible_sel))
                for i, ev in enumerate(sorted(visible_sel, key=lambda x: x["year_start"])):
                    with tab_cols[i]:
                        if st.button("↑", key=f"tab_{ev['id']}", use_container_width=True,
                                     help=f"Show details for {ev['name']}"):
                            st.session_state["detail_focus"] = ev["id"]
